...
def run_network_command(command):
    # This method attempts to directly invoke 'cmd.exe' with parameters that ensure visibility
    try:
        # The 'start' command is prefixed to ensure a new window is opened
        # '/k' option is used to keep the window open after the command executes
        command_to_run = f'cmd.exe /c start cmd.exe /k "{command} && echo. && pause"'
        subprocess.Popen(command_to_run, shell=True)
    except Exception as e:
        show_message(f"Failed to execute network command: {e}")
...
mainWindow.setWindowTitle('System Utility Commands')
